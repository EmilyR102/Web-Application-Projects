...
export function renderBarChart(data) {
    ...
    //     'x',
    //     ([d]) =>
    //         countryScale(d.data.country_name) + countryScale.bandwidth() / 3
    // )
    // .attr(
    //     'y',
    //     ([d]) => statScale(d[1]) + (statScale(d[0]) - statScale(d[1])) / 2
    // )
    // .text(([d]) => (d[1] - d[0]).toLocaleString('en-US'))
